...
int main(int argc, char *argv[])
{
  ...
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1), &mem_us);
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1), &mem_vs);
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1), &mem_ws);
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1), &mem_qs);
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1), &mem_rho_i);
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1), &mem_square);
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1), &mem_forcing);
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1) * 5, &mem_u);
  brisbane_mem_create(sizeof(double) * KMAX * (JMAXP + 1) * (IMAXP + 1) * 5, &mem_rhs);
  ...
  ny = grid_points[1];
  nz = grid_points[2];
  #pragma omp target update to(nx,ny,nz,dt,grid_points)
...
//#pragma omp target data map(alloc:forcing,rho_i,u,us,vs,ws,square,qs,rhs) //create(u,forcing,rho_i)
{
  initialize();
  ...
  brisbane_mem_release(mem_us);
  brisbane_mem_release(mem_vs);
  brisbane_mem_release(mem_ws);
  brisbane_mem_release(mem_qs);
  brisbane_mem_release(mem_rho_i);
  brisbane_mem_release(mem_square);
  brisbane_mem_release(mem_forcing);
  brisbane_mem_release(mem_u);
  brisbane_mem_release(mem_rhs);
